...
export class SceneService {
   ...
   * 2. 从 _gltfLoader.components 提取真实顶点/三角形
   * 3. 用 MeshAnalyzer 计算凸包体积、表面积、实心度等
   * 4. 若提取失败，降级到 scene.sampleHeightMostDetailed 高度采样
   */
  async analyzeAt(
    ...
    // === 优先路径：真实网格几何提取 ===
    try {
      const picked = this.geometryExtractor.extractFromScreen(viewer.scene, position)
      if (picked) {
        const bbox = this.buildBBoxFromMesh(picked)
        if (bbox) return bbox
      }
    } catch (e) {
  ...
  /**
   * 从提取的网格几何构建 BoundingInfo
   */
  private buildBBoxFromMesh(picked: {
  ...
  async autoScan(
    ...
    // === 优先路径：遍历已加载瓦片提取真实几何 ===
    try {
      const tileMeshes = this.geometryExtractor.extractAllFromTileset(this.tileset)
      if (tileMeshes.length > 0) {
        const results: BoundingInfo[] = []
        for (const tm of tileMeshes.slice(0, maxObjects * 3)) {
          const bbox = this.buildBBoxFromMesh(tm)
